...
export function tryBuildCurrentTurnCapabilityReply({
  ...
  return [
    `Current turn route: ${route} (${apiStyle} / ${model})`,
    `Callable tools: ${formatToolLabels(effectiveTools)}`,
    `Suppressed tools: ${formatSuppressedTools(suppressedTools)}`,
    `Web search: ${formatBooleanStatus(requestDescriptor.effectiveEnableWebSearch)}`,
    `Code interpreter: ${formatBooleanStatus(requestDescriptor.effectiveEnableCodeInterpreter)}`,
    'This is computed per-turn rather than being a fixed global list.'
  ].join('\n');
}
